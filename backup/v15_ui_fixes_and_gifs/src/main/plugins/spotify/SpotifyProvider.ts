...
export class SpotifyProvider implements IMusicProvider {
    ...
    try {
      ...
      if (!token) {
        ...
        token = await this.authManager.startOAuthFlow(this.id, {
          clientId: '6c5423dfd5db43a2b5eb95b80c637706',
          scopes: ['user-read-private', 'streaming', 'user-library-read', 'playlist-read-private'],
  ...
  async getLikedTracks(): Promise<Track[]> {
    try {
      const data = await this.httpClient.get<any>('/me/tracks?limit=50');
      if (!data || !data.items) return [];
      ...
      return data.items
        ...
        .map((item: any) => ({
          ...
          provider: this.id,
          likedAt: item.added_at ? new Date(item.added_at).getTime() : undefined
        }));
    } catch (e) {
