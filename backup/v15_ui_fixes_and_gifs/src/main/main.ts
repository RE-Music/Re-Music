...
app.whenReady().then(() => {
  ...
  ipcMain.handle('search', async (_: any, { providerId, query, page = 1 }: { providerId: string, query: string, page?: number }) => {
    ...
    if (providerId === 'all') {
      ...
      const results = await Promise.allSettled(
        authenticatedProviders.map((p: any) => 
      ...
      Logger.info(`[IPC] Aggregated search success: ${allResults.length} results from ${authenticatedProviders.length} providers`);
      return allResults;
    }
  ...
  ipcMain.handle('get-liked-tracks', async (_: any, providerId?: string) => {
    ...
    if (providerId) {
      ...
      Logger.info(`[IPC] Aggregated liked tracks success: ${allTracks.length} tracks (sorted by date)`);
      return allTracks;
    }
  });
