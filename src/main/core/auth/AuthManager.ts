...
export class AuthManager {
  ...
  async startOAuthFlow(providerId: string, config: {
    ...
    Logger.info(`[AuthManager] startOAuthFlow started for ${providerId}`);
    return new Promise((resolve, reject) => {
       const authWindow = new BrowserWindow({
         ...
         webPreferences: { 
           ...
           partition: `persist:auth_${providerId}`
         }
       });
       ...
       try {
         require('fs').writeFileSync('c:/Users/User/Nmis/auth_debug.txt', authUrl);
       } catch (e: any) {
         Logger.info(`[AuthManager] Debug file write failed: ${e.message}`);
       }
       Logger.info(`[AuthManager] Starting OAuth flow for ${providerId}. URL: ${authUrl.replace(config.clientId, 'REDACTED')}`);

       const stealthUA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/133.0.0.0 Safari/537.36';
       authWindow.webContents.session.setUserAgent(stealthUA);
    ...
    return new Promise((resolve) => {
      const authWindow = new BrowserWindow({
        ...
        webPreferences: {
          nodeIntegration: false,
          contextIsolation: true,
          partition: 'persist:auth_yandex'
        }
      });

      const mobileUA = 'Mozilla/5.0 (Linux; Android 13; SM-G960F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Mobile Safari/537.36';
      authWindow.webContents.session.setUserAgent(mobileUA);
