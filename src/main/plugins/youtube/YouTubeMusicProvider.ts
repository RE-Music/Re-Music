...
export class YouTubeMusicProvider implements IMusicProvider {
  ...
  private async callApi(endpoint: string, body: Record<string, any> = {}, customClient: 'WEB' | 'IOS' | 'WEB_REMIX' | 'TVHTML5' | 'ANDROID_MUSIC' | 'ANDROID_VR' = 'WEB_REMIX'): Promise<any> {
    const worker = await this.getWorkerWindow();
    const baseUrl = (customClient === 'ANDROID_VR' || customClient === 'IOS' || customClient === 'ANDROID_MUSIC') 
      ? 'https://youtubei.googleapis.com' 
      : YTM_BASE;
    const url = `${baseUrl}/youtubei/v1/${endpoint}`;
    ...
    if (customClient === 'IOS') {
      ...
      credentials = 'omit';
    } else if (customClient === 'ANDROID_VR') {
      ctx = {
        client: {
  ...
  async play(trackId: string): Promise<string> {
    ...
    // Attempt playback using ANDROID_VR client (bypasses LOGIN_REQUIRED and poToken)
    let data = await this.callApi('player', {
      videoId,
      playbackContext: { contentPlaybackContext: { signatureTimestamp: 19876 } }
    }, 'ANDROID_VR');

    let streamingData = data?.streamingData;

    // Fallback 1: WEB_REMIX client
    if (!streamingData || data?.playabilityStatus?.status !== 'OK') {
      Logger.info(`[YouTubeMusicProvider] ANDROID_VR playback status: ${data?.playabilityStatus?.status || 'FAILED'}, trying WEB_REMIX...`);
