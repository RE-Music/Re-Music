  ...
  async getLikedTracks(): Promise<Track[]> {
    ...
    try {
      ...
      if (likedItems[0].track) {
        ...
        // Если только ID, нужно загрузить данные для ВСЕХ треков чанками по 200
        Logger.info(`[YandexProvider] Hydrating details for ${likedItems.length} liked tracks in chunks...`);
        const timestamps = new Map(likedItems.map((i: any) => [String(i.id), i.timestamp]));
        const allTrackIds = likedItems.map((item: any) => item.id);
        const allFetchedTracks: Track[] = [];

        for (let i = 0; i < allTrackIds.length; i += 200) {
          const chunk = allTrackIds.slice(i, i + 200).join(',');
          const tracksData = await this.httpClient.post<any>('/tracks', `track-ids=${chunk}`, {
            headers: { 
              'Content-Type': 'application/x-www-form-urlencoded',
              'X-Yandex-Music-Client': 'WindowsPhone/1.39'
            }
          ...
          if (tracksData.result) {
            const mappedChunk = tracksData.result.map((track: any) => {
              const albumId = track.albums?.[0]?.id;
              const fullId = albumId ? `${track.id}:${albumId}` : String(track.id);
            ...
            allFetchedTracks.push(...mappedChunk);
          }
        }
        
        Logger.info(`[YandexProvider] Successfully hydrated ${allFetchedTracks.length} tracks.`);
        return allFetchedTracks;
      }
    } catch (e) {
